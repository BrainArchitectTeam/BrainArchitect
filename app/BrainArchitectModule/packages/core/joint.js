(function(global){	// BEGIN CLOSURE
    ...
    /**
     *Sort of optimalization, delaying invocation by 0 ms makes rendering faster (that's what she said)
     *@param function
     *@return {boolean}
     */
    ...
    /**
     *Is object Array? Not implemented in certain browsers.
     *@param {Object} 
     *@return {boolean}
     */
    ...
    /**
     *Is object simple Object? Not implemented in certain browsers.
     *@param {Object} 
     *@return {boolean}
     */
 ...
 */
     ...
     */
        ...
        this._opt = {
            ...
            overlayAttrs: {"fill-opacity": 0,"stroke": "#f00","stroke-opacity": 0,"stroke-width": 10}
            ,
            ...
            boundPoint: {
                start: null,        // from where to start connection (brabec)
                end: null           // where to end connection (brabec)
            },
        ...
        labelAttrs: [
            ...
            {position: 1/2, fill: 'black', 'font-size': 10, offset: 0},
            {position: 20, fill: 'black', 'font-size': 10, offset: 10},
            {position: -20, fill: 'black', 'font-size': 10, offset: 10},
        ...
        data: {
            ...
            name: '',                       // name of association (represents label)
        ...
        this._opt.arrow = {
            start: Joint.getArrow("none", 0),//, this._opt.attrs),
            end: Joint.getArrow("none", 0)
        };
        // options
        this._opt = DeepMixin(this._opt, opt);
 ...
 */
    ...
    ////////////////////////////////////////////////////////////////////////////
    // Connection editor
    // add Franta Toman 18. dubna
    
    /**
     *  Connection editor
     */
    Joint.connectionEditor = null;

    Joint.setConnectionEditor = function(connectionEditor){         
        Joint.connectionEditor = connectionEditor;
    };
    
    Joint.getConnectionEditor = function(){
        return Joint.connectionEditor;
    };
    
    Joint.connectionTypeSelector = null;
    
    Joint.setConnectionTypeSelector = function(connectionSelector){
        Joint.connectionTypeSelector = connectionSelector;
    };
    
    Joint.getConnectionTypeSelector = function(){
        return Joint.connectionTypeSelector;
    };
 ...
 */
     ...
     */
        objectContainingPoint: function(p){
            // in future versions of raphael; now the last added element has privilege to connect when overlapped by another (brabec)
            
            /*
            var o = this.paper.getElementByPoint(p.x,p.y);
            if (!o) return null;
            console.log(o);
            return o;
            */
           
           /* VERSION COMMENTED ON 6. APRIL BY FRANTA TOMAN
            ...
            */
            ...
            while (idx--){
                o = register[idx].yourself();
                if (rect(o.getBBox()).containsPoint(p)){
                    intersect.push(register[idx]);                  
                }
            }
            
            var elementsCount = intersect.length;
            ...
            for( var i = 0; i < elementsCount - 1; i++ ){
                if( winner.getDepth() > intersect[i+1].getDepth() ){
                    winner = intersect[i+1];
                }
            }
            
            return winner.yourself();
     ...
     */
        capMouseDown: function(e, cap){
            ...
            var mousePos = Joint.getMousePosition(e,Joint.paper());
            this._dx = mousePos.x;//e.clientX;
            this._dy = mousePos.y;//e.clientY;
            this.select();
            //console.log("capMouseDown");
            if (cap === this.dom.startCap){
                //this.disconnect("start");  // moved to capMouseMove (brabec)
                this.state = this.STARTCAPDRAGGING;
            } else if (cap === this.dom.endCap){
                //this.disconnect("end"); // moved to capMouseMove
                this.state = this.ENDCAPDRAGGING;
            }
        },
        
        /**
         *Show Connection edit dialog (toman, brabec)
         */
        connectionDoubleclick: function(e){
            
            var editor = Joint.getConnectionEditor();
            
            if( editor != null ){
                editor.setReference(this);
                editor.open();                
            }
        ...
        /**
         * Set type of connection (brabec)
         * @param type One of Joint.dia arrows
         */
        ...
        setConnectionType: function(type){
            ...
            this.processOptions(type);
            this.update();
        },
        
        /**
         *Set cardinalities of connection (brabec)
         *@param card {String} representing cardinality value
         *@param pos {String} start|end 
         */
        setConnectionCardinality: function(card, pos){
            var lidx;
            if(pos=="start") lidx = 1;
            else if(pos=="end") lidx = 2;
            else return;
            
            var label = this._opt.label;
            label[lidx] = card;
            this.processOptions(label);
            this.update();
        },
         ...
         *@param pos {String} start|end 
         */
        setConnectionRole: function(card, pos){
            var lidx;
            if(pos=="start") lidx = 3;
            else if(pos=="end") lidx = 4;
            else return;
        ...
        /**
         *Set middle connection label
         */
        setConnectionLabel: function(text){
        ...
        updateConnectionLabel: function(){
            ...
            var name = this._opt.data.name;
            var stereotype = this._opt.data.stereotype;
            ...
            this.setConnectionLabel(label);
        },
        
        ////////////////////////////////////////////////////////////////////////
        // DATA MANIPULATION ADDED BY FRANTA TOMAN 18. dubna 2012
        
        setConnectionAttrs: function(attrs){
            this.processOptions(attrs);
            this.update();
        },
        
        setName: function(name){
            this._opt.data.name = name;
            this.updateConnectionLabel();
        },
        
        setStereotype: function(stereotype){
            this._opt.data.stereotype = stereotype;
            this.updateConnectionLabel();
        },
        
        setType: function(type){
            this._opt.data.type = type;
        },
        
        setSourceData: function(data){
            this._opt.data.source.multiplicity = data['multiplicity'];
            this._opt.data.source.name = data['name'];
            this._opt.data.source.role = data['role'];
            
            this.setConnectionCardinality(data['multiplicity'], "start");
            this.setConnectionRole(data['role'], "start");
            
        },
        
        setTargetData: function(data){           
            this._opt.data.target.multiplicity = data['multiplicity'];
            this._opt.data.target.name = data['name'];
            this._opt.data.target.role = data['role'];
            
            this.setConnectionCardinality(data['multiplicity'], "end");
            this.setConnectionRole(data['role'], "end");
        },
        
        getName: function(){
            return this._opt.data.name;
        },
        
        getStereotype: function(){
            return this._opt.data.stereotype;
        },
        
        getType: function(){
            return this._opt.data.type;
        },
        
        getSourceData: function(){
            return this._opt.data.source;
        },
        
        getTargetData: function(){
            return this._opt.data.target;
        },
     ...
     */
        connectionMouseDown: function(e){
            ...
            //doubleclick
            if(Joint.dia._doubleclick){
                this.connectionDoubleclick(e);
                return;
            }else{
                Joint.dia._doubleclick = this;
                setTimeout(function(){Joint.dia._doubleclick = false;},300);
            }
        ...
        capDragging: function(e){
            // move dummy object
            //var mousePos = point(e.clientX,e.clientY);
            var mousePos = Joint.getMousePosition(e,this.paper);
            var sRect = rect(this.startObject().getBBox());
            var eRect = rect(this.endObject().getBBox());
            
            if (this.state === this.STARTCAPDRAGGING && this._start.dummy){
                //console.log("dragging start dummy");
                this.startObject().translate(mousePos.x - this._dx, mousePos.y - this._dy);
                //this.startObject().translate(e.clientX - this._dx, e.clientY - this._dy);
            } else if (this.state === this.ENDCAPDRAGGING && this._end.dummy) {
                //console.log("dragging end dummy");
                //this.endObject().translate(e.clientX - this._dx, e.clientY - this._dy);
                this.endObject().translate(mousePos.x - this._dx, mousePos.y - this._dy);
            }
            
            var translateBy;
            if (this.state === this.STARTCAPDRAGGING && !this._start.dummy ) {
                if(mousePos.distance(sRect.pointNearestToPoint(mousePos))>50){
                    this.disconnect("start");
                    translateBy = mousePos.relativeTo(rect(this.startObject().getBBox()).center());
                    this.startObject().translate(translateBy.x, translateBy.y);
                    this._opt.boundPoint.start = null;
                   // console.log("start disconnected");
                    //return;
                }else{
                    //console.log("dragging start on outline");
                    this._opt.boundPoint.start = sRect.boundPoint(sRect.pointNearestToPoint(mousePos)).relativeTo(sRect.center());
                }
                //this.update();
                //return;
            } else if (this.state === this.ENDCAPDRAGGING && !this._end.dummy ) {
                if(mousePos.distance(eRect.pointNearestToPoint(mousePos))>50){
                    this.disconnect("end");
                    translateBy = mousePos.relativeTo(rect(this.endObject().getBBox()).center());
                    this.endObject().translate(translateBy.x, translateBy.y);
                    this._opt.boundPoint.end = null;
                    //console.log("end disconnected");
                    //return;
                } else {
                    //console.log("dragging end on outline");
                    this._opt.boundPoint.end = eRect.boundPoint(eRect.pointNearestToPoint(mousePos)).relativeTo(eRect.center());;
                }
                //this.update();
                //return;
            }
        ...
        capEndDragging: function(){
            ...
            if (!o) {this.liquidate();return;}
            if (o && this["_" + capType].dummy){
                ...
                if( this.startObject() ){
                    this.startObject().wholeShape.select();
                }
                
                ////////////////////////////////////////////////////////////////////////
                // soo set attributes of connection by ConnectionTypeSelectorActiveItem

                var cts = Joint.getConnectionTypeSelector();                        // cts = Connection Type Selector

                if( cts != null && cts != undefined ){

                    var kind = cts.getSelectedType();
                    this.setType( kind.type );
                    this.setConnectionAttrs( kind.attrs );

                    //if( kindonSet )

                    if( typeof(kind.onSet) == 'function'){
                        kind.onSet( this );
                    }
        ...
        redraw: function(){
            this.clean(["connection","connectionOverlay", "startCap", "endCap", "handleStart", "handleEnd", "label"]);
            this.draw(["connection","connectionOverlay", "startCap", "endCap", "handleStart", "handleEnd", "label"]);
            return this;
        },
        listenAll: function(){
            ...
            var i;
            i = this.dom.connectionOverlay.length;
            while (i--) {
                this.dom.connectionOverlay[i].mousedown(function(e){
                    Joint.fixEvent(e);
     ...
     */
        boundPoint: function(bbox, type, rotation, p){
            ...
            else if (type === 'rect' && bbox.width == bbox.height && rotation && rotation != 0) {
     ...
     */
        jointLocation: function(start, end, vertices, boundPoint){
            ...
            if(boundPoint.start){ //(brabec)
                // compute from relative boundpoint
                var startBoundPoint = boundPoint.start;
                var sbp = point(start.bbox.center().x + startBoundPoint.x,start.bbox.center().y + startBoundPoint.y);
                //update start boundboint
                var sbbox = start.bbox;
                boundPoint.start = sbbox.pointNearestToPoint(sbp).relativeTo(sbbox.center());
            }
            
            if(boundPoint.end){
                // compute from relative boundpoint
                var endBoundPoint = boundPoint.end;
                var ebp = point(end.bbox.center().x + endBoundPoint.x, end.bbox.center().y + endBoundPoint.y);
                //update end boundboint
                var ebbox = end.bbox;
                boundPoint.end = ebbox.pointNearestToPoint(ebp).relativeTo(ebbox.center());
            }
            
            
            // start object boundary point
            p1bp = sbp || this.boundPoint(start.bbox, start.type, start.rotation, firstVertex || ebp || end.bbox.center());
            // shift
            theta = (sbp || start.bbox.center()).theta(ebp || firstVertex || end.bbox.center());
            // first point of the connection
            p1 = p1bp,// point(p1bp.x + (1 * start.shift.dx * cos(theta.radians)),p1bp.y + (-1 * start.shift.dy * sin(theta.radians)));
            ...
            p2bp = ebp || this.boundPoint(end.bbox, end.type, end.rotation, lastVertex || sbp || start.bbox.center());
            // shift
            theta = (sbp || lastVertex || start.bbox.center()).theta(ebp || end.bbox.center());
            // last point of the connection
            p2 = p2bp, //point(p2bp.x + (-1 * end.shift.dx * cos(theta.radians)),p2bp.y + (1 * end.shift.dy * sin(theta.radians)));
     ...
     */
        clean: function(components){
            ...
            while (idx--){
                ...
                if (component && !component.removed){ // !removed (brabec)
                    if (component.node){
                        component.remove();
                        this.dom[name] = null;
            ...
            return this; // support chaining (brabec :) )
        },

        /**
     * Process options.
     * @todo Please fix me! I look like spaghetti.
     * @private
     * @param {object} opt
     */
        processOptions: function(opt){
            ...
            // boundPoints (brabec)
            if(opt.boundPoint){
                myopt.boundPoint.start = opt.boundPoint.start;
                Mixin(myopt.boundPoint.start, opt.boundPoint.start);
                myopt.boundPoint.end = opt.boundPoint.end;
                Mixin(myopt.boundPoint.end, opt.boundPoint.end);
            }
        },
        // Public API
     ...
     */
        highlight: function(color){
            color = color || "red";
            //this.connection().attr("stroke-width", 2);
            return this;
        },
     ...
     */
        unhighlight: function(){
            ...
            this.connection().attr("stroke-width", 1);
            return this;
        },
        liquidate: function(){ //(brabec)
            
            this.deselect();
            this.freeJoint(this.startObject());
            this.freeJoint(this.endObject());
            this.clean(["connection","connectionOverlay", "startCap", "endCap", "handleStart", "handleEnd", "label","dummyEnd","dummyStart"]);
            Joint.dia.unregisterJoint(this);   
            var i = this._registeredObjects.length;
            while(i--){
                this.unregister(this._registeredObjects[i]);
            }
 ...
 */
    ...
    Joint.addEvent = function(element, type, handler){
        if (element.addEventListener) {
            ...
            if (!handler.$$guid){
                handler.$$guid = Joint.addEvent.guid++;
            }
    ...
    //Joint.addEvent(Joint.paper().canvas, "mousedown", Joint.mouseDown);
    ...
    var JointDOMBuilder = {
        ...
        handleStart: function(){
            ...
            return this.paper.rect(pos.x-(opt.radius/2), pos.y-(opt.radius/2), opt.radius, opt.radius).attr(opt.attrs);
        },
        ...
        connectionOverlay: function(){// brabec
            var opt = this.opt, paths = [];
            //console.log(nopt);
            var con = this.paper.path(this.connectionPathCommands.join(" ")).attr(opt.overlayAttrs);
            if (opt.subConnectionAttrs) {
                var i = 0, l = opt.subConnectionAttrs.length, 
                length = con.getTotalLength();
                for (; i < l; i++) {
        ...
        label: function(){
            ...
            for (; i < len; i++) {
                ...
                if( labels[i] != '' && labels[i] != null ){
                    ...
                    var bbText = labelText.getBBox();                
                    var labelBox =  this.paper.rect(bbText.x, bbText.y, bbText.width, bbText.height).attr(this.opt.labelBoxAttrs[i]);

                    labelText.insertAfter(labelBox);

                    components.push(labelBox);
 ...
 */
     ...
     */
        adhereToRect: function(r){
            ...
            var x = mmin(mmax(this.x, r.x), r.x + r.width);
            var y = mmin(mmax(this.y, r.y), r.y + r.height);
            return point(x,y);
        },
     ...
     */
        ...
        relativeTo: function(p){ // relative position of me reffering to p (brabec)
 ...
 */
    ...
    Rect.prototype = {
        ...
        center: function(){
            return point(this.x + Math.floor(this.width/2), this.y + Math.floor(this.height/2));
        },
     ...
     */
        intersect: function(r){
            ...
            if (rCorner.y <= myOrigin.y){
                return false;
            }
            if (rOrigin.x >= myCorner.x){
                return false;
            }
            if (rOrigin.y >= myCorner.y){
                return false;
            }
            return true;
        },
     ...
     */
        boundPoint: function(p){
            var center = this.center();//point(this.x + this.width/2, this.y + this.height/2);
            ...
            for (i = sides.length - 1; i >= 0; --i){
                intersection = sides[i].intersection(connector);
                if (intersection !== null){
                    return intersection;
                }
            }*/
